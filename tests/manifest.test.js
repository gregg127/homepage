...
describe("manifest.webmanifest", () => {
  ...
  before(() => {
    assert.ok(
      ...
      "public/manifest.webmanifest is missing",
    );
  ...
  it("start_url is absolute or root-relative", () => {
    assert.match(
      manifest.start_url,
      /^(https?:\/\/|\/)/,
      "start_url must be absolute or root-relative",
    );
  });
  ...
  it("every icon entry has src, sizes, and type", () => {
    const incomplete = manifest.icons.filter(
      (icon) => !icon.src || !icon.sizes || !icon.type,
    );
    assert.deepEqual(
      incomplete,
      [],
      `icons missing required fields: ${JSON.stringify(incomplete)}`,
    );
  });
