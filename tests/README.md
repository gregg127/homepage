...
| `pages.test.js`           | Each page has correct `<title>`, meta description, `lang="en"`, canonical URL, OG/Twitter tags and `og:image`; only 404 is `noindex` |
...
| `structured-data.test.js` | Homepage JSON-LD is valid JSON describing a schema.org `Person` with alma mater and `sameAs` social links                            |
