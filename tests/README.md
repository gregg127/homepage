...
| `pages.test.js`           | Each built HTML page has the correct `<title>`, a non-empty `<meta name="description">`, `lang="en"`, canonical URL, Open Graph and Twitter Card tags, `og:image` exists with declared size; only 404 is `noindex` |
| `html-validity.test.js`   | DOM-level checks via cheerio: exactly one `<h1>`, heading hierarchy, every `<img>` has alt, internal links resolve, no localhost in production HTML |
