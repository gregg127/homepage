...
const collectRefs = ($) => {
  ...
  $("script[src]").each((_, el) =>
    refs.push({ kind: "script", url: $(el).attr("src") }),
  );
  $("link[href]").each((_, el) =>
    refs.push({ kind: "link", url: $(el).attr("href") }),
  );
  $("img[src]").each((_, el) =>
    refs.push({ kind: "img", url: $(el).attr("src") }),
  );
  $("source[src]").each((_, el) =>
    refs.push({ kind: "source", url: $(el).attr("src") }),
  );
  $("video[src]").each((_, el) =>
    refs.push({ kind: "video", url: $(el).attr("src") }),
  );
  return refs;
};
...
for (const file of PAGES) {
  describe(`asset integrity: ${file}`, () => {
    ...
    const localRefs = refs.filter(
      (r) => r.url && !isExternal(r.url) && r.url.startsWith("/"),
    );

    it("references at least one local asset", () => {
      assert.ok(
        localRefs.length > 0,
        `no local asset references found in ${file}`,
      );
    });
